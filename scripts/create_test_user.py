...
async def initialize_team(teamname, password, problem_number):
    ...
    insert_and_return = f"""
        INSERT INTO user_details_2023(username, password) VALUES ('{teamname}', '{password}') RETURNING user_id
    """
    ...
    insert_rankings = f"""INSERT INTO rankings_2023(team_id, score) VALUES ($1, 0)"""
    await conn.execute(create_table)
    await conn.execute(insert_query)
    await conn.execute(insert_rankings, team_id)

run_async(initialize_team("UltraScienceGuy", "tombradyfans", 35))