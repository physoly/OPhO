...
def get_score(attempts, day_solved, num_teams_solved, question_num):
    c1 = pow(0.9, attempts + day_solved)
    c2 = exp(question_num/40) + max(5.5 - floor(log(num_teams_solved)), 2)
    return c1 * c2

async def execute():
    conn = await get_connection()
    team_ids = [team_id[0] for team_id in await conn.fetch('SELECT user_id from user_details')]
    ...
    for team_id, data in all_team_stats.items():
        for problem in data:
            if problem['solved']:
                ...
                raw_score = get_score(attempts=num_attempts, day_solved=day_number, num_teams_solved=num_solved, question_num=problem_no)
                scores[team_id] = scores[team_id] + round(raw_score, 3)

    sorted_scores = sorted(scores.items(), key=lambda kv: kv[1], reverse=True)
    
    query = await conn.prepare(f'SELECT username from user_details where user_id=$1')
